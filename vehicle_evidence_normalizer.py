...
SOURCES = {"telegram", "divar", "bama", "sheypoor", "karnameh", "formula", "khodro45", "hamrah_mechanic"}
...
class VehicleEvidence:
    ...
    @property
    def chassis_hit(self) -> int | None:
        # Front/rear location is intentionally collapsed. Any established chassis
        # damage is one unified defect point; healthy is zero; partial/unknown/
        # conflicting evidence remains unknown.
        if self.chassis.status == "healthy_claim":
            return 0
        if self.chassis.status == "damaged_claim":
            return 1
        return None

    @property
    def defect_score(self) -> int | None:
        """Unified Stage 3 defect score without invented facts.

        Each explicitly counted painted panel = 1 point, each explicitly counted
        replacement panel = 1 point, and any established chassis damage = 1 point
        total regardless of front/rear/both. Full-paint and unknown/conflicting
        evidence do not receive an invented numeric count, so the total is None.
        """
        paint = self.paint.count if self.paint.status in {"clean", "counted"} else None
        replacement = (self.replacements.count
                       if self.replacements.status in {"clean", "counted"} else None)
        chassis = self.chassis_hit
        if paint is None or replacement is None or chassis is None:
            return None
        return int(paint) + int(replacement) + int(chassis)
...
def normalize_vehicle(*, source: str, source_id: str, url: str, title: str, raw_text: str,
    ...
    price, pr = numeric.parse_vehicle_price(text, allow_shorthand=source == "telegram")
...
def normalize_retained(runtime: Path) -> dict:
    ...
    try:
        ...
        for row in observations:
            ...
            payload["chassis_hit"] = item.chassis_hit
            payload["defect_score"] = item.defect_score
            payload["stage3_identity_complete"] = bool(item.brand and item.model and item.trim and item.model_year)
...
def run_self_test() -> None:
    ...
    count += 1
    # Stage 3 unified defect scoring: location does not matter; chassis front/rear
    # is one point total and replacement is counted exactly like paint.
    score_cases = [
        ("یک گلگیر رنگ\nشاسی سالم\nبدون تعویض", 1),
        ("یک کاپوت رنگ\nشاسی سالم\nبدون تعویض", 1),
        ("بدون رنگ\nشاسی جلو ضربه دارد\nبدون تعویض", 1),
        ("بدون رنگ\nشاسی عقب ضربه دارد\nبدون تعویض", 1),
        ("بدون رنگ\nشاسی جلو و عقب ضربه دارد\nبدون تعویض", 1),
        ("بدون رنگ\nشاسی سالم\nیک در تعویضی", 1),
        ("دو قطعه رنگ\nشاسی ضربه دارد\nیک در تعویضی", 4),
    ]
    for body_text, expected in score_cases:
        x = normalize_vehicle(source="telegram", source_id="score", url="https://t.me/test/2",
                              title="پژو 206 تیپ 5",
                              raw_text="پژو 206 تیپ 5\nمدل 1397\nکارکرد 120000\nقیمت 1250\n" + body_text)
        if x.defect_score != expected:
            raise AssertionError(f"stage3 defect score failed: {body_text!r} => {x.defect_score}, expected {expected}")
        count += 1
    unknown = normalize_vehicle(source="telegram", source_id="score2", url="https://t.me/test/3",
                                title="پژو 206 تیپ 5",
                                raw_text="پژو 206 تیپ 5\nمدل 1397\nکارکرد 120000\nقیمت 1250")
    if unknown.defect_score is not None:
        raise AssertionError("Unknown defect evidence must not be coerced to zero")
    count += 1
