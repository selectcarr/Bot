...
def process_batch(settings: Settings, store: Store, results: Iterable[CollectorResult], *,
    ...
    LOGGER.info(
        "%s SharedBatchComplete candidates=%s deals=%s messages=%s message_limit=%s "
        "message_limit_scope=whole_run engine_mode=authority reference_method=median",
        SYSTEM, len(admitted), len(deals), messages, settings.max_messages_per_run,
    )
...
def _public_run_config(settings):
    return {
        ...
        "message_limit": settings.max_messages_per_run,
        "message_limit_scope": "whole_run_all_selected_sources",
...
def run() -> int:
    ...
    try:
        ...
        LOGGER.info(
            ...
            "allow_send=%s max_messages_per_run=%s",
