...
def _deal_from_authority(listing: NormalizedVehicleListing, decision) -> Deal | None:
...
def process_batch(settings: Settings, store: Store, results: Iterable[CollectorResult], *,
    ...
    for listing in admitted:
        ...
        deal = _deal_from_authority(listing, decision)
    ...
    for deal in deals[:settings.max_messages_per_run]:
        LOGGER.info(
            ...
            "price=%s discount=%.2f source=%s scope=shared_batch engine_mode=authority",
            SYSTEM, deal.listing.comparison_key, deal.sample_count, deal.reference_price,
            deal.reference_method, deal.listing.price, deal.discount_percent, deal.listing.source,
        )
        ...
        if allow_send:
            ...
            messages += 1
            LOGGER.info(
                "%s TelegramSent source_key=%s message_id=%s reference_method=median engine_mode=authority",
