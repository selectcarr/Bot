...
from select_carr_banks import SelectCarrBanks, snapshot_from_service_deal
...
def process_batch(settings: Settings, store: Store, results: Iterable[CollectorResult], *,
    ...
    # Stage 2 business bank is deliberately separate from the pricing/reference DB.
    # Historical deal events must not be pruned with aa_listings.
    business_bank = SelectCarrBanks(settings.runtime_dir / "select_carr_business.db")
    business_bank.initialize()
    ...
    # Persist every newly detected deal into the business bank before publication.
    # The pricing engine remains authoritative for detection; this is storage only.
    for deal in deals:
        business_bank.record_deal(snapshot_from_service_deal(deal))
    ...
    for deal in deals[:settings.max_messages_per_run]:
        ...
        if allow_send:
            ...
            deal_id = business_bank.record_deal(snapshot_from_service_deal(deal))[0]
            business_bank.mark_deal_published(deal_id, "telegram", message_id)
            messages += 1
