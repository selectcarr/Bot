...
def _identity_token(value: object) -> str:
    # Keep the same normalization contract as NormalizedVehicleListing without
    # importing private collector details here.
    from accurate_average_collectors import normalize_for_match
    return normalize_for_match(value)


STAGE3_REFERENCE_VERSION = "1.0.0"


def _known_int(value: object) -> int | None:
    return value if isinstance(value, int) and not isinstance(value, bool) else None


def _stage3_rank(candidate, row: dict) -> tuple:
    """Rank references inside an exact identity group without hard-dropping unknowns.

    Priority is user-defined: mileage first, then unified defect score. Missing
    secondary evidence is lower priority, never fabricated and never a hard reject.
    Price is deliberately absent from the ranking.
    """
    ckm = _known_int(getattr(candidate, "mileage", None))
    rkm = _known_int(row.get("mileage"))
    if ckm is not None and rkm is not None:
        mileage_rank = (0, abs(ckm - rkm))
    elif ckm is None and rkm is not None:
        mileage_rank = (0, 0)  # candidate has no mileage: do not penalize known references
    else:
        mileage_rank = (1, 0)

    cscore = _known_int(getattr(candidate, "defect_score", None))
    rscore = _known_int(row.get("defect_score"))
    if cscore is None:
        defect_rank = (0, 0) if rscore is not None else (1, 0)
    elif rscore == cscore:
        defect_rank = (0, 0)
    elif rscore is None:
        defect_rank = (1, 0)
    else:
        defect_rank = (2, abs(cscore - rscore))

    stamp = _timestamp(row.get("first_seen"))
    freshness = -(stamp.timestamp() if stamp is not None else 0.0)
    return mileage_rank + defect_rank + (freshness, str(row.get("source_key", "")))


def _stage3_select_rows(candidate, rows: Iterable[dict], maximum: int) -> list[dict]:
    # Identity filtering happens before this helper. It only prioritizes secondary
    # evidence and never changes the 3..10 policy.
    return sorted((dict(row) for row in rows), key=lambda row: _stage3_rank(candidate, row))[:maximum]
...
class Store:
    ...
    def _backup_before_stage3_reference_migration(self):
        """Snapshot the market DB once before Stage 3 grouping migration."""
        if not self.path.is_file():
            return
        source = destination = None
        temporary = None
        try:
            ...
            if "aa_listings" not in tables:
                return
            columns = {row[1] for row in source.execute("PRAGMA table_info(aa_listings)")}
            if "defect_score" in columns and "evidence_state" in columns:
                return
            folder = self.path.parent / "state_backups"
            folder.mkdir(parents=True, exist_ok=True)
            final = folder / "before_stage3_reference_v1.sqlite3"
            if final.exists():
                return
            handle, name = tempfile.mkstemp(prefix="before_stage3_reference_", suffix=".tmp", dir=folder)
            os.close(handle)
            temporary = Path(name)
            destination = sqlite3.connect(temporary)
            source.backup(destination)
            if destination.execute("PRAGMA quick_check").fetchone()[0] != "ok":
                raise RuntimeError("Stage 3 migration backup check failed")
            destination.close(); destination = None
            os.replace(temporary, final); temporary = None
            LOGGER.info("%s Stage3Migration backup_created=true", SYSTEM)
        finally:
    ...
    def initialize(
        ...
        self._backup_before_stage3_reference_migration()
        ...
        with self.connect() as connection:
            ...
            for name, declaration in additions.items():
                if name not in columns:
                    connection.execute(f"ALTER TABLE aa_listings ADD COLUMN {name} {declaration}")
            # Stage 3 changes the durable base group to exact identity only. Legacy
            # body/condition values are NOT converted into defect points. Existing
            # rows therefore retain unknown Stage 3 evidence until fresh normalized
            # evidence is observed. No TTL is introduced and this migration does not
            # bulk-prune consolidated groups.
            rows = connection.execute(
                "SELECT source_key,brand,model,trim,model_year,comparison_key FROM aa_listings"
            ).fetchall()
            for row in rows:
                identity_key = "|".join((
                    _identity_token(row["brand"]), _identity_token(row["model"]),
                    _identity_token(row["trim"]), str(row["model_year"]),
                ))
                if identity_key != row["comparison_key"]:
                    connection.execute("UPDATE aa_listings SET comparison_key=? WHERE source_key=?",
                                       (identity_key, row["source_key"]))
    ...
    def reference_rows(self, listing: NormalizedVehicleListing) -> list[dict]:
        # Stage 3 production selector: exact brand/model/trim/year base group;
        # mileage and unified defects are ranking signals, not hard requirements.
        with self.connect() as connection:
            rows = [dict(row) for row in connection.execute(
                "SELECT * FROM aa_listings WHERE comparison_key=? AND source_key<>?",
                (listing.comparison_key, listing.source_key),
            ).fetchall()]
        return _stage3_select_rows(listing, rows, self.max_samples)
            ...
            if existing:
                connection.execute(
                    ...
                    (
                        ...
                        received_text,
                        listing.paint_count,
                        listing.replacement_count,
                        listing.chassis_hit,
                        listing.defect_score,
                        listing.evidence_state,
                        listing.mileage,
                        listing.price,
...
class Deal:
    ...
    # Stage 5 observability only: aggregate source counts for the exact
    # authoritative samples selected by the engine. No source enters grouping.
    reference_sources: tuple[tuple[str, int], ...] = ()

    @property
    def average_price(self) -> int:
        # Compatibility only: the persisted column is historically named
        # average_price, but Stage 4 production value is the median reference.
...
def evaluate_against_history(
    ...
    prior = (store.reference_rows(listing)
             if hasattr(store, "reference_rows")
             else store.prior_group_rows(listing.comparison_key, exclude_source_key=listing.source_key))
    ...
    return Deal(
        ...
        reference_method="arithmetic_mean_legacy_audit",
...
def format_message(
    ...
    condition = (
        "صفر" if deal.listing.condition == "zero"
        else "کارکرده" if deal.listing.condition == "used"
        else "نامشخص"
    )
    ...
    reference_sources = "، ".join(
        f"{source_labels.get(source, source)}: {count}"
        for source, count in deal.reference_sources
    ) or "نامشخص"
    ...
    return (
        ...
        f"{deal.sample_count} "
        "نمونه مشابه:\n"
        f"{deal.reference_price:,} "
        "تومان\n\n"

        "🧾 منابع نمونه‌های مرجع:\n"
        f"{reference_sources}\n\n"
...
def send_telegram(
    ...
    endpoint = "https://api.telegram.org/" f"bot{settings.bot_token}/"

    response = None
    if use_photo:
        # Telegram cannot always fetch the temporary/public image URL itself.
        # Download the exact image that the collector already identified, then
        # upload the bytes directly. No image content is inferred here.
        try:
            photo_response = requests.get(
                ...
                timeout=(8, min(30, settings.timeout_seconds)),
                allow_redirects=True,
                stream=True,
            )
            photo_response.raise_for_status()
            content_type = str(photo_response.headers.get("content-type", "")).split(";", 1)[0].strip().lower()
            if not content_type.startswith("image/"):
                raise RuntimeError("Telegram deal photo URL did not return an image")
            image = bytearray()
            for chunk in photo_response.iter_content(16_384):
                image.extend(chunk)
                if len(image) > 9_000_000:
                    raise RuntimeError("Telegram deal photo exceeds upload limit")
            if not image:
                raise RuntimeError("Telegram deal photo is empty")
            ...
            response = requests.post(
                ...
                files={
                    "photo": ("deal" + suffix, bytes(image), content_type or "image/jpeg"),
                },
                timeout=settings.timeout_seconds,
            )
            LOGGER.info(
                "%s TelegramPhotoUpload source=direct_bytes bytes=%s content_type=%s",
                SYSTEM, len(image), content_type or "unknown",
            )
        except (requests.RequestException, RuntimeError) as exc:
            # Never lose a confirmed deal only because its photo could not be
            # transferred. Fall back to the established text-only send path.
            LOGGER.warning(
                "%s TelegramPhotoFallback reason=%s text_send=true",
...
# Stage 4 keeps one shared reference set independent of source completion order.
# Universal Engine v3 is the only production deal authority; legacy mean is audit-only.
SHARED_BATCH_VERSION = "1.1.0"
...
def _batch_row(listing: NormalizedVehicleListing, first_seen: str) -> dict:
    return {
        ...
        "paint_count": listing.paint_count, "replacement_count": listing.replacement_count,
        "chassis_hit": listing.chassis_hit, "defect_score": listing.defect_score,
        "evidence_state": listing.evidence_state,
...
class _SharedReferenceView:
    ...
    def reference_rows(self, listing: NormalizedVehicleListing) -> list[dict]:
        rows = self.all_group_rows(listing.comparison_key, listing.source_key)
        return _stage3_select_rows(listing, rows, self.max_samples)
...
def _prepare_shared_batch(settings: Settings, store: Store, results: Iterable[CollectorResult]):
    ...
    for listing in ordered:
        ...
        if tokens & seen_tokens:
            ...
            continue
        # Stage 4 suspicious-low-price guard uses the SAME authoritative median
        # engine against PRE-RUN history. No legacy mean may veto or admit a row.
        pre_history = original.all_group_rows(listing.comparison_key, listing.source_key)
        pre_decision = _universal_replay(listing, pre_history)
        if pre_decision.reason == "discount_above_maximum":
            suspicious_count += 1
            LOGGER.info("%s SharedBatchReject source=%s reason=suspicious_low_price reference_method=median",
                        SYSTEM, listing.source)
            continue
    ...
    LOGGER.info(
        ...
        "reference_pool=history_plus_current_batch production_reference=median stage3_reference=identity_plus_soft_secondary engine_mode=authority",
...
def _trace_shared_decision(settings: Settings, store: Store, view: _SharedReferenceView,
    ...
    """Log authoritative median decision plus legacy mean audit; never alter it."""
    try:
        all_rows = view.all_group_rows(listing.comparison_key, listing.source_key)
        decision = _universal_replay(listing, all_rows)
        legacy_rows = view.reference_rows(listing)
        legacy, _ = _legacy_reason(listing, legacy_rows, store.min_samples,
                                   settings.threshold_percent, settings.max_deal_percent)
        selected = set(decision.sample_keys)
        distribution = dict(sorted(Counter(
            row["source"] for row in all_rows
            if str(row.get("source_key", "")) in selected
        ).items()))
        LOGGER.info(
            "%s SharedDecision source=%s source_key=%s authority_reason=%s authority_samples=%s "
            "reference_method=%s reference_sources=%s authority_details=%s "
            "legacy_audit_reason=%s already_sent=%s allow_send=%s engine_mode=authority",
            SYSTEM, listing.source, listing.source_key, decision.reason, decision.sample_count,
            decision.reference_method, json.dumps(distribution, sort_keys=True, separators=(",", ":")),
            ",".join(decision.details) or "none", legacy,
...
def _assert_authority_policy(settings: Settings, store: Store) -> None:
    """Fail closed if environment and authoritative engine policy drift apart."""
    expected = (
        universal_engine.MIN_SAMPLES,
        universal_engine.MAX_SAMPLES,
        universal_engine.MIN_DEAL_PERCENT,
        universal_engine.MAX_DEAL_PERCENT,
    )
    actual = (
        store.min_samples,
        store.max_samples,
    ...
    if actual != expected:
        raise RuntimeError(
            "Stage 4 authority policy mismatch: service must be 3..10 samples and 1..15 percent"
        )


def _deal_from_authority(listing: NormalizedVehicleListing, decision, all_rows) -> Deal | None:
    if not decision.is_deal or decision.market_average is None or decision.discount_percent is None:
        return None
    if decision.reference_method != "median":
        raise RuntimeError("Authoritative engine returned a non-median reference")
    selected = set(decision.sample_keys)
    ...
    if sum(distribution.values()) != int(decision.sample_count):
        raise RuntimeError("Stage 5 reference-source accounting mismatch")
    return Deal(
        listing=listing,
        reference_price=int(decision.market_average),
        discount_percent=round(float(decision.discount_percent), 2),
        sample_count=int(decision.sample_count),
        reference_method=decision.reference_method,
        reference_sources=tuple(sorted((source, int(count)) for source, count in distribution.items())),
    )
...
def process_batch(settings: Settings, store: Store, results: Iterable[CollectorResult], *,
    ...
    """Stage 4 production path: Universal Engine v3 is the sole deal authority.

    The legacy arithmetic mean remains observable only for audit. It cannot create,
    suppress, rank, or send a deal. References are exact identity groups from Stage 3,
    then ranked by mileage and unified defect score inside the authoritative engine.
    """
    if allow_send and not write_state:
        raise ValueError("Sending requires durable sent-message memory")
    _assert_authority_policy(settings, store)
    ...
    for listing in admitted:
        ...
        deal = _deal_from_authority(listing, decision, all_rows)
        if deal is not None and not store.was_sent(listing.source_key):
            deals.append(deal)

    # All decisions are frozen before durable writes/pruning.
    if write_state:
    ...
    for deal in deals:
        LOGGER.info(
            "%s DealFound group=%s samples=%s reference_price=%s reference_method=%s "
            "reference_sources=%s price=%s discount=%.2f source=%s "
            "scope=shared_batch engine_mode=authority stage5_e2e=true",
            SYSTEM, deal.listing.comparison_key, deal.sample_count, deal.reference_price,
            deal.reference_method, json.dumps(dict(deal.reference_sources), sort_keys=True, separators=(",", ":")),
            deal.listing.price, deal.discount_percent, deal.listing.source,
        )
        text = format_message(deal)
        if allow_send:
            message_id = send_telegram(
                settings, text, getattr(deal.listing, "image_url", "")
            )
            # Historical DB column name is average_price; Stage 4 stores the median
            # reference value there for backward schema compatibility.
            store.mark_sent(deal.listing, deal.reference_price, deal.discount_percent)
            messages += 1
            LOGGER.info(
                "%s TelegramAPIResponse ok=true source_key=%s message_id=%s media=%s "
                "reference_method=median engine_mode=authority stage5_e2e=true",
                SYSTEM, deal.listing.source_key, message_id,
                "photo" if getattr(deal.listing, "image_url", "") and len(text) <= 1000 else "text",
            )
        else:
            print("\n--- ACCURATE SYSTEM B DRY RUN ---\n" + text + "\n--- END ---\n")
    LOGGER.info(
        "%s SharedBatchComplete candidates=%s deals=%s messages=%s message_policy=all_confirmed_unsent "
        "engine_mode=authority reference_method=median",
        SYSTEM, len(admitted), len(deals), messages,
    )
...
# Stage 4: read-only audit retains the legacy mean only as a comparator.
STAGE4_AUTHORITY_VERSION = "1.0.0"
STAGE5_E2E_VERSION = "1.0.0"
SERVICE_AUDIT_VERSION = "2.0.0"
AUDIT_LOOKBACK_DAYS = 4
AUDIT_MAX_ROWS = 10000


def _engine_contract() -> None:
    if getattr(universal_engine, "ENGINE_VERSION", None) != "3.0.0":
        raise RuntimeError("Stage 4 requires universal_deal_engine version 3.0.0")
...
def _row_listing(row):
    """Adapt stored fields explicitly; missing secondary evidence stays unknown."""
    keys = set(row.keys()) if hasattr(row, "keys") else set(row)
    required = {key: row[key] for key in (
        "source", "source_ad_id", "url", "brand", "model", "trim",
        "model_year", "condition", "body_condition", "mileage", "price",
    )}
    required["defect_score"] = row["defect_score"] if "defect_score" in keys else None
    required["evidence_state"] = row["evidence_state"] if "evidence_state" in keys else "unknown"
...
def _public_run_config(settings):
    return {
        ...
        "legacy_reference_method": "arithmetic_mean_audit_only",
        "production_reference_method": "median",
        "production_engine": "universal_deal_engine_v3",
        "production_engine_mode": "authority_to_send",
        "stage4_authority_version": STAGE4_AUTHORITY_VERSION,
        "stage5_e2e_version": STAGE5_E2E_VERSION,
        ...
        "message_policy": "all_confirmed_unsent",
        "legacy_message_limit_ignored": settings.max_messages_per_run,
        "processing_mode": "collect_all_then_compare",
        "shared_batch_version": SHARED_BATCH_VERSION,
        "stage3_reference_version": STAGE3_REFERENCE_VERSION,
        "comparison_scope": "all_system_b_sources",
        "base_group": "brand_model_trim_year",
        "secondary_priority": "mileage_then_unified_defect_score",
        "unified_defect_score": "paint_panels_plus_replacement_panels_plus_any_chassis_hit_once",
        ...
        "production_engine_unchanged": False,
    }
...
def _trace_decision(settings, store, listing, allow_send):
    ...
    try:
        ...
        LOGGER.info(
            ...
            "already_sent=%s allow_send=%s mode=authority",
...
def _emit_audit(settings, report):
    ...
    LOGGER.info("%s AuditStatus=%s mode=read_only_no_source_requests engine_mode=authority", SYSTEM, report["status"])
...
def _run_service_bridge_self_test():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if not _universal_replay(replace(candidate, mileage=None), prior).is_deal:
            raise AssertionError("Unknown mileage must remain identity-comparable in Stage 4")
        if not _universal_replay(replace(candidate, body_condition="unknown"), prior).is_deal:
            raise AssertionError("Unknown body must remain identity-comparable in Stage 4")
    ...
    print("service universal authority integration self-test: OK")
...
def _run_stage3_reference_self_test() -> None:
    """Offline regression for Stage 3 bank migration and hierarchical selection."""
    with tempfile.TemporaryDirectory() as directory:
        path = Path(directory) / "test.db"
        store = Store(path, 3, 10)
        store.initialize()
        base = dict(source="telegram", title="fixture", brand="Peugeot", model="206", trim="تیپ 2",
                    model_year=1399, condition="unknown", body_condition="unknown",
                    price=1_000_000_000, collected_at=now_iso(), raw_text="")
        candidate = NormalizedVehicleListing(source_ad_id="c", url="https://t.me/x/1", mileage=None,
                    paint_count=1, replacement_count=0, chassis_hit=0, defect_score=1,
                    evidence_state="scored", **base)
        refs = [
            NormalizedVehicleListing(source="bama", source_ad_id="r1", url="https://bama.ir/car/detail-r1",
                title="r1", brand="Peugeot", model="206", trim="تیپ 2", model_year=1399,
                condition="used", body_condition="clean", mileage=90000, price=1_000_000_000,
                collected_at=now_iso(), raw_text="", paint_count=0, replacement_count=1,
                chassis_hit=0, defect_score=1, evidence_state="scored"),
            NormalizedVehicleListing(source="divar", source_ad_id="r2", url="https://divar.ir/v/x/r2",
                title="r2", brand="Peugeot", model="206", trim="تیپ 2", model_year=1399,
                condition="used", body_condition="accident", mileage=None, price=1_020_000_000,
                collected_at=now_iso(), raw_text="", paint_count=0, replacement_count=0,
                chassis_hit=1, defect_score=1, evidence_state="scored"),
            NormalizedVehicleListing(source="karnameh", source_ad_id="r3", url="https://karnameh.com/used-cars/00000000-0000-0000-0000-000000000000",
                title="r3", brand="Peugeot", model="206", trim="تیپ 2", model_year=1399,
                condition="unknown", body_condition="unknown", mileage=None, price=980_000_000,
                collected_at=now_iso(), raw_text="", defect_score=None, evidence_state="partial"),
        ]
        for item in refs:
            store.upsert(item)
        selected = store.reference_rows(candidate)
        if len(selected) != 3 or selected[0]["defect_score"] != 1:
            raise AssertionError("Stage 3 hierarchical reference selection failed")
        if candidate.comparison_key != refs[0].comparison_key:
            raise AssertionError("Secondary condition/body fields leaked into identity key")
        deal = evaluate_against_history(store, replace(candidate, price=950_000_000), 1.0, 15.0)
        if deal is None or deal.sample_count != 3 or deal.average_price != 1_000_000_000:
            raise AssertionError("Stage 3 identity fallback or arithmetic mean changed")
        different = replace(candidate, model_year=1398, source_ad_id="other")
        if store.reference_rows(different):
            raise AssertionError("Different model year must never enter the base group")
    print("service stage3 reference self-test: OK")
    print(f"service stage3 reference version={STAGE3_REFERENCE_VERSION} base_group=brand_model_trim_year")
...
def _run_shared_batch_self_test() -> None:
    ...
    class SharedBatchTests(unittest.TestCase):
        ...
        def test_10_coarse_body_is_not_a_stage3_hard_gate(self):
            candidate = self.item("telegram", "target", 950_000_000)
            refs = [replace(item, body_condition="minor_paint") for item in self.refs()]
            _, view = self.plan([candidate] + refs)
            self.assertEqual(len(view.reference_rows(candidate)), 3)

        def test_10b_unified_defect_score_prioritizes_equal_score(self):
            candidate = replace(self.item("telegram", "target", 950_000_000),
                                mileage=None, paint_count=1, replacement_count=0,
                                chassis_hit=0, defect_score=1, evidence_state="scored")
            same = [replace(self.item("bama", f"same{i}"), mileage=100000+i,
                            paint_count=0, replacement_count=1, chassis_hit=0,
                            defect_score=1, evidence_state="scored") for i in range(3)]
            other = [replace(self.item("divar", f"other{i}"), mileage=100000+i,
                             paint_count=0, replacement_count=0, chassis_hit=0,
                             defect_score=0, evidence_state="scored") for i in range(3)]
            _, view = self.plan([candidate] + other + same)
            selected = view.reference_rows(candidate)
            self.assertEqual([row["defect_score"] for row in selected[:3]], [1, 1, 1])

        def test_10c_unknown_secondary_evidence_falls_back_to_identity(self):
            candidate = replace(self.item("telegram", "target", 950_000_000), mileage=None,
                                body_condition="unknown", defect_score=None, evidence_state="partial")
            refs = [replace(item, mileage=None, defect_score=None, evidence_state="partial") for item in self.refs()]
            _, view = self.plan([candidate] + refs)
            result = self.evaluate(view, candidate)
        ...
        def test_13_all_confirmed_deals_are_sent_and_history_saved(self):
            items = [self.item("telegram", "target", 950_000_000)] + self.refs()
            ...
            with patch(__name__ + ".send_telegram", return_value=1) as sender:
                ...
                self.assertEqual((deals, messages, sender.call_count), (2, 2, 2))
            with self.store.connect() as conn:
                self.assertEqual(conn.execute("SELECT COUNT(*) FROM aa_listings").fetchone()[0], 8)
                self.assertEqual(conn.execute("SELECT COUNT(*) FROM aa_sent_deals").fetchone()[0], 2)
        ...
        def test_15_authority_uses_median_not_legacy_mean(self):
            candidate = self.item("telegram", "target", 950_000_000)
            refs = self.refs()
            ...
            self.assertEqual(decision.market_average, 1_000_000_000)
            self.assertEqual(decision.reference_method, "median")

        def test_16_authority_allows_unknown_secondary_evidence_before_ten_cap(self):
            candidate = self.item("telegram", "target", 950_000_000)
            items = [candidate] + self.refs() + [self.item("bama", f"missingkm{i}", mileage=None) for i in range(12)]
            _, view = self.plan(items)
            decision = _universal_replay(candidate, view.all_group_rows(candidate.comparison_key, candidate.source_key))
            self.assertTrue(decision.is_deal)
            self.assertEqual(decision.sample_count, 10)
            self.assertIn("over_sample_limit=5", decision.details)
        ...
        def test_21b_legacy_mean_cannot_control_production_send(self):
            candidate = self.item("telegram", "authority", 950_000_000)
            items = [candidate] + self.refs()
            with patch(__name__ + ".evaluate_against_history", side_effect=AssertionError("legacy authority forbidden")), \
                 patch(__name__ + ".send_telegram", return_value=77) as sender:
                deals, sent = process_batch(self.settings, self.store, self.results(items),
                                            write_state=True, allow_send=True)
            self.assertEqual((deals, sent, sender.call_count), (1, 1, 1))
    ...
    print(f"service shared-source batch version={SHARED_BATCH_VERSION} tests={outcome.testsRun} engine_mode=authority reference_method=median")
...
def _stage3_profile_from_payload(payload: dict) -> dict:
    """Extract only explicit Stage 3 defect evidence from normalized payload."""
    paint = payload.get("paint") if isinstance(payload.get("paint"), dict) else {}
    repl = payload.get("replacements") if isinstance(payload.get("replacements"), dict) else {}
    ...
    def count_of(claim):
        ...
        if status == "clean":
            return 0
        if status == "counted" and isinstance(value, int) and not isinstance(value, bool) and value >= 0:
            return value
        return None

    paint_count = count_of(paint)
    replacement_count = count_of(repl)
    chassis_status = chassis.get("status")
    chassis_hit = 0 if chassis_status == "healthy_claim" else 1 if chassis_status == "damaged_claim" else None
    defect_score = (paint_count + replacement_count + chassis_hit
                    if paint_count is not None and replacement_count is not None and chassis_hit is not None
                    else None)
    return {
        ...
        "evidence_state": "scored" if defect_score is not None else "partial",
    }


def _apply_stage3_reference_profiles(settings: Settings, results: Iterable[CollectorResult]) -> tuple[CollectorResult, ...]:
    """Attach Stage 2 evidence to this run without inventing missing facts.

    Missing/partial evidence stays eligible for identity-level comparison. A payload
    whose vehicle identity disagrees with the accepted listing is ignored.
    """
    results = tuple(results)
    path = settings.runtime_dir / vehicle_evidence_normalizer.DATABASE_NAME
    payloads = {}
    if path.is_file():
        connection = sqlite3.connect(path.resolve().as_uri() + "?mode=ro", uri=True, timeout=10)
        try:
            connection.row_factory = sqlite3.Row
            connection.execute("PRAGMA query_only=ON")
            for row in connection.execute("SELECT source_key,payload_json FROM vehicle_normalizations"):
                try:
                    payload = json.loads(row["payload_json"])
                except (TypeError, ValueError):
                    continue
                if isinstance(payload, dict):
                    payloads[str(row["source_key"])] = payload
        finally:
            connection.close()

    enriched = []
    scored = partial = missing = conflict = 0
    for result in results:
        items = []
        for listing in result.listings:
            payload = payloads.get(listing.source_key)
            if payload is None:
                items.append(replace(listing, evidence_state="missing"))
                missing += 1
                continue
            same_identity = (
                _identity_token(payload.get("brand")) == _identity_token(listing.brand)
                and _identity_token(payload.get("model")) == _identity_token(listing.model)
                and _identity_token(payload.get("trim")) == _identity_token(listing.trim)
                and payload.get("model_year") == listing.model_year
            )
            if not same_identity:
                items.append(replace(listing, evidence_state="identity_conflict"))
                conflict += 1
                continue
            profile = _stage3_profile_from_payload(payload)
            items.append(replace(listing, **profile))
            if profile["defect_score"] is None:
                partial += 1
            else:
                scored += 1
        enriched.append(CollectorResult(
            result.source, result.fetched, result.accepted, result.rejected, result.duplicates,
            result.blocked, result.error, tuple(items),
        ))
    LOGGER.info(
        "%s Stage3ReferenceProfiles version=%s scored=%s partial=%s missing=%s identity_conflict=%s "
        "base_group=brand_model_trim_year secondary_priority=mileage_then_unified_defect_score",
        SYSTEM, STAGE3_REFERENCE_VERSION, scored, partial, missing, conflict,
    )
...
def _run_stage5_e2e_self_test() -> None:
    """Offline regression for decision -> message -> send-memory observability.

    Real source/detail HTTP is intentionally not used in self-test. The live/dry-run
    service path performs that I/O; this test proves the final authoritative handoff,
    exact reference-source accounting, Telegram response logging contract and
    duplicate-send memory without requiring credentials.
    """
    import io
    from contextlib import redirect_stdout
    from dataclasses import replace
    ...
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        settings = replace(
            load_settings(), runtime_dir=root, database_path=root / "test.db",
            diagnostics_dir=root / "diagnostics", log_path=root / "test.log",
            action="live", live_enabled=True, bot_token="test", chat_id="test",
            min_samples=3, max_samples=10, threshold_percent=1.0,
            max_deal_percent=15.0, max_messages_per_run=1,
        )
        store = Store(settings.database_path, 3, 10)
        store.initialize()
        stamp = "2026-01-01T00:00:00+00:00"

        def item(source, key, price):
            return NormalizedVehicleListing(
                source=source, source_ad_id=key,
                url=f"https://example.test/{source}/{key}", title=f"fixture {source} {key}",
                brand="Peugeot", model="206", trim="تیپ 5", model_year=1397,
                condition="used", body_condition="unknown", mileage=None,
                price=price, collected_at=stamp, raw_text="",
                defect_score=None, evidence_state="partial",
            )

        candidate = replace(
            item("telegram", "candidate", 950_000_000),
            image_url="https://cdn.example.test/car.jpg",
        )
        refs = [
            item("divar", "r1", 1_000_000_000),
            item("bama", "r2", 1_000_000_000),
            item("karnameh", "r3", 1_000_000_000),
        ]
        grouped = {}
        for row in [candidate, *refs]:
            grouped.setdefault(row.source, []).append(row)
        results = tuple(
            CollectorResult(source, len(rows), len(rows), 0, 0, False, None, tuple(rows))
            for source, rows in grouped.items()
        )

        with patch(__name__ + ".send_telegram", return_value=777) as sender, \
             patch.object(LOGGER, "info") as logs, redirect_stdout(io.StringIO()):
            deals, messages = process_batch(settings, store, results, write_state=True, allow_send=True)
            if (deals, messages, sender.call_count) != (1, 1, 1):
                raise AssertionError("Stage 5 authoritative send handoff failed")
            sent_text = sender.call_args.args[1]
            if sender.call_args.args[2] != candidate.image_url:
                raise AssertionError("Stage 5 did not pass Telegram photo provenance to sender")
            for expected in ("دیوار: 1", "باما: 1", "کارنامه: 1", "قیمت مرجع 3"):
                if expected not in sent_text:
                    raise AssertionError("Stage 5 message is missing reference provenance: " + expected)
            emitted = repr(logs.call_args_list)
            if "TelegramAPIResponse ok=true" not in emitted or "message_id=%s" not in emitted:
                raise AssertionError("Stage 5 Telegram response observability missing")

        if not store.was_sent(candidate.source_key):
            raise AssertionError("Stage 5 sent-message memory was not persisted")
        ...
        with patch.object(requests, "get", return_value=_PhotoResponse()) as getter, \
             patch.object(requests, "post", return_value=_Response()) as post:
            if send_telegram(settings, "photo caption", candidate.image_url) != 888:
                raise AssertionError("Telegram photo response was not parsed")
            if getter.call_args.args[0] != candidate.image_url:
                raise AssertionError("Telegram photo was not fetched from listing provenance")
            url = post.call_args.args[0]
            data = post.call_args.kwargs.get("data", {})
            files = post.call_args.kwargs.get("files", {})
            if not url.endswith("/sendPhoto") or data.get("caption") != "photo caption" or "photo" not in files:
                raise AssertionError("Telegram direct photo upload contract failed")

        with patch(__name__ + ".send_telegram", side_effect=AssertionError("duplicate send forbidden")), \
             redirect_stdout(io.StringIO()):
            deals, messages = process_batch(settings, store, results, write_state=True, allow_send=True)
            if messages != 0:
                raise AssertionError("Stage 5 duplicate-send memory failed")

    print("service stage5 end-to-end handoff self-test: OK")
    print("service stage5 e2e version=1.0.0 authority=median reference_provenance=true telegram_response=true duplicate_send_memory=true")
...
def run() -> int:
    ...
    try:
        ...
        # Stage 3 attaches whatever secondary evidence was actually established.
        # Missing mileage/body/chassis/replacement data is not fabricated and does
        # not disqualify an otherwise exact identity match.
        batch_results = list(_apply_stage3_reference_profiles(settings, batch_results))
        ...
        total_deals, total_messages = process_batch(
            settings, store, batch_results, write_state=True, allow_send=allow_send,
        )
        LOGGER.info(
            "%s Stage5E2E version=%s action=%s sources=%s fetched=%s accepted=%s "
            "detail_status=%s normalization_status=%s deals=%s messages=%s "
            "allow_send=%s message_policy=all_confirmed_unsent legacy_max_messages_per_run=%s",
            SYSTEM, STAGE5_E2E_VERSION, settings.action, ",".join(sources),
            total_fetched, total_accepted, detail_error or "ok",
            normalization_error or "ok", total_deals, total_messages,
            allow_send, settings.max_messages_per_run,
        )
