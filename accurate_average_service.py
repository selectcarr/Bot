...
def send_telegram(
    ...
    response = requests.post(
        "https://api.telegram.org/" f"bot{settings.bot_token}/" + method,
        json=payload,
...
def _run_stage5_e2e_self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        with patch.object(requests, "post", return_value=_Response()) as post:
            ...
            if not url.endswith("/sendPhoto") or payload.get("photo") != candidate.image_url:
                raise AssertionError("Telegram sendPhoto contract failed")
