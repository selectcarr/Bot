...
class TelegramCollector(
    ...
    def _ocr_image(self, source_key: str, image_url: str) -> str:
        """Best-effort Persian/English OCR. Failure never fabricates evidence."""
        ...
        try:
            ...
            result = subprocess.run(
                ["tesseract", str(temporary), "stdout", "-l", "fas+eng", "--psm", "6"],
                stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True, timeout=30, check=False,
            )
            ocr_text = source_normalizers_v2.normalize_text(result.stdout)[:8000] if result.returncode == 0 else ""
        ...
        try:
            ...
            cache.close()
        LOGGER.info("%s TelegramImageEvidence post=%s status=%s chars=%s",
        ...
        for channel in (
            ...
            try:
                ...
                for (
                    ...
                    recoverable = (
                        reason.startswith("invalid_price:")
                        or reason.startswith("invalid_year:")
                        or reason in {"unknown_vehicle", "missing_trim", "missing_text"}
                    )
                    evidence_text = text
                    if listing is None and image_url and recoverable and ocr_attempts < self.OCR_MAX_PER_RUN:
                        ocr_attempts += 1
                        ocr_text = self._ocr_image(key, image_url)
                        if ocr_text:
                            evidence_text = "\n".join(x for x in (text, ocr_text) if x).strip()
                            title = (text.splitlines()[0] if text else ocr_text.splitlines()[0])[:300]
                            listing, reason = normalize_listing(
                                source=self.source, source_ad_id=key, url=post_url,
                                title=title, raw_text=evidence_text,
                            )
                            if listing is not None:
                                listing = replace(
                                    listing, image_url=image_url, image_evidence_used=True,
                                    raw_text=evidence_text[:4000],
                                )
    ...
    with patch.object(requests.sessions.Session, "request", forbidden_network):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                require(ocr_spy.call_count == 1, "Image OCR must be invoked only for incomplete text")
            finally:
                collector.close()
