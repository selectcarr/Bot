...
def load_settings() -> Settings:
    ...
    settings = Settings(
        ...
        max_ads_per_run=get_int(
            "DIVAR_MAX_ADS_PER_RUN",
            60,
        ),
        retention_days=get_int(
            "DIVAR_RETENTION_DAYS",
            7,
        ),

        min_sample_count=get_int(
            "DIVAR_MIN_SAMPLE_COUNT",
            3,
        ),
        min_deal_percent=get_float(
            "DIVAR_MIN_DEAL_PERCENT",
            2.0,
        ),
        max_deal_percent=get_float(
        ...
        first_page_delay_max=get_int(
            "DIVAR_FIRST_PAGE_DELAY_MAX",
            6,
        ),
        next_page_delay_min=get_int(
            "DIVAR_NEXT_PAGE_DELAY_MIN",
            3,
        ),
        next_page_delay_max=get_int(
            "DIVAR_NEXT_PAGE_DELAY_MAX",
            7,
        ),

        request_timeout_seconds=get_int(
