...
def main():
    ...
    print(f"[debug] طول توکن: {len(bot_token) if bot_token else 0} | شروع: {bot_token[:10] if bot_token else 'خالی'} | پایان: {bot_token[-5:] if bot_token else 'خالی'}", file=sys.stderr)
    print(f"[debug] chat_id: {repr(chat_id)}", file=sys.stderr)
    ...
    print(f"[info] تعداد دیلهای مناسب: {len(deals)}")
