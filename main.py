...
------------------------------------------
ویژگی‌های این نسخه:
1. نام خودرو: زودترین مدل شناخته‌شده‌ای که واقعاً در متن ظاهر شده انتخاب می‌شود
   (نه اولین مورد در لیست config)، با مرز کلمه دقیق تا با کلمات مشابه قاطی نشود.
2. سال مدل: هم فرمت کامل (۱۳۹۸/۱۴۰۲/۲۰۲۴) و هم مخفف دو‌رقمی (۹۸، ۰۵) پشتیبانی می‌شود.
3. لیست روزانه‌ی قیمت/ارز فقط بین ساعت ۱۲:۳۰ تا ۱۵:۳۰ به وقت ایران فرستاده می‌شود.
4. فیلد «قیمت صفر» فقط از لیست پرفروش کانال yv_vk خوانده می‌شود؛ اگر پیدا نشد،
   بدون هیچ منبع جایگزین دیگری، «پیدا نشد» نوشته می‌شود.
5. آگهی‌های صفرکیلومتر فقط با صفرهای هم‌نام+هم‌مدل مقایسه می‌شوند (نه با کارکرده‌ها).
6. آگهی‌های قبلاً فرستاده‌شده دوباره ارسال نمی‌شوند (sent_ids.json).
"""
...
def is_within_daily_window():
    """لیست روزانه فقط بین ساعت ۱۲:۳۰ تا ۱۵:۳۰ به وقت ایران مجاز است."""
    now = datetime.now(IRAN_TZ)
    minutes = now.hour * 60 + now.minute
    return 750 <= minutes <= 930
...
def extract_car_name(text, car_models):
    # زودترین مدلی که واقعاً در متن ظاهر شده انتخاب می‌شود؛ اگر چند مدل از
    # یک نقطه شروع شدند (مثل «بنز» داخل «مرسدس بنز»)، طولانی‌ترین/دقیق‌ترین
    # مورد ترجیح داده می‌شود تا نام کامل خودرو از دست نرود.
...
def parse_model_year(text):
    # اعداد فارسی به انگلیسی تبدیل می‌شود، سپس هم سال کامل (۱۳۹۸/۱۴۰۲/۲۰۲۴)
    # و هم مخفف دو‌رقمی (۹۸ یعنی ۱۳۹۸، ۰۵ یعنی ۱۴۰۵) تشخیص داده می‌شود.
...
def build_zero_km_price_table(listings):
    """
    از میان آگهی‌های صفرکیلومتر شناسایی‌شده در کانال‌های آگهی، میانگین قیمت
    هر (نام+سال) و هر (نام) را می‌سازد. این جدول فقط برای مقایسه‌ی داخلی گروه
    آگهی‌های صفر (در analyze_listings) استفاده می‌شود، نه برای فیلد «قیمت صفر»
    نمایشی که فقط از لیست روزانه‌ی yv_vk خوانده می‌شود.
    """
    by_name_year = {}
    by_name_only = {}
...
def lookup_zero_km_price(car_name, yv_price_table):
    """
    قیمت صفر فقط از لیست روزانه‌ی کانال yv_vk خوانده می‌شود.
    اگر خودرو در آن لیست نبود، «پیدا نشد» برمی‌گردد (بدون هیچ منبع جایگزین).
    """
...
def build_yv_price_table(price_list_text, car_models):
    ...
    فیلد «قیمت صفر» استفاده می‌شود، نه برای محاسبه دیل‌ها.
    """
    table = {}
...
def strip_source_signature(text):
    """
    امضای انتهایی کانال مرجع (مثل یک خط تنها با @username) را حذف می‌کند
    تا به‌جای آن آدرس ادمین پشتیبانی خودمان قرار بگیرد.
    """
