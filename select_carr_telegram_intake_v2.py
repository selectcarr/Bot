...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        encrypted_existed = ENC_DB.exists()
        decrypt_db(db_path)
        ...
        # Encrypt only when the plaintext DB was created or actually changed.
        if (not encrypted_existed) or (before_hash != after_hash):
            encrypt_db(db_path)

        # Persist Telegram cursor only when updates were actually observed.
        if max_seen >= offset:
    ...
    print(json.dumps({
        ...
        "ignored":ignored,
        "errors":errors,
        "db_changed": (not encrypted_existed) or (before_hash != after_hash),
