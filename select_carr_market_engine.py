...
def process_execution(mc,system_db,obs_db,e,force_candidates=False):
    ...
    for row in obs:
        ...
        if ch or force_candidates: changed.append(row)
...
def run_engine(system_db=SYSTEM_DB,obs_db=OBS_DB,market_db=MARKET_DB,allow_send=ALLOW_SEND):
    ...
    with db(market_db) as mc:
        ...
        if first:
            ...
            # v1.0.0 first-run bug recovery: bootstrap seeded the latest accepted
            # rows before candidate detection, producing changed_candidates=0.
            # Re-evaluate that already-processed latest execution exactly once.
            if sget(mc,"bootstrap_candidate_recovery_v101") != "1":
                latest=latest_execution(system_db)
                if latest is not None and latest.id == last:
                    executions=[latest] + executions
                    first=True
                sset(mc,"bootstrap_candidate_recovery_v101","1")
        reports=[]
        for e in executions:
            rep=process_execution(mc,system_db,obs_db,e,force_candidates=first); reports.append(rep)
        ...
        sset(mc,"engine_version",ENGINE_VERSION)
...
class Tests(unittest.TestCase):
    ...
    def test_24_first_boot_forces_latest_candidates_after_seed(self):
        n=now_utc()
        cand=fx("candidate",900,source="telegram",when=n)
        refs=[fx(f"r{i}",1000,source=s,when=n-timedelta(hours=1)) for i,s in enumerate(("divar","bama","karnameh"))]
        self.put(cand,*refs)
        parsed={"brand":cand["brand"],"model":cand["model"],"trim":cand["trim"],
                "model_year":cand["model_year"],"condition":cand["condition"],
                "body_condition":cand["body_condition"],"mileage":cand["mileage"],"price":cand["price"]}
        start=iso(n-timedelta(minutes=5)); finish=iso(n+timedelta(minutes=1))
        with sqlite3.connect(self.o) as c:
            c.execute("INSERT INTO listing_observations VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                      (cand["source_key"],cand["source"],cand["source_ad_id"],cand["url"],cand["title"],
                       "raw","h",3,0,"s","accepted","accepted","clean",
                       json.dumps(parsed,ensure_ascii=False),start,iso(n)))
        e=Execution(178,start,finish,"scheduled",1)
        with db(self.m) as c:
            rep=process_execution(c,self.s,self.o,e,force_candidates=True)
            statuses=[r[0] for r in c.execute("SELECT status FROM market_deals")]
        self.assertEqual(rep["changed_candidates"],1)
        self.assertEqual(rep["evaluated"],1)
        self.assertEqual(rep["queued"],1)
        self.assertEqual(statuses,["pending"])

    def test_25_without_force_seeded_unchanged_row_is_not_candidate(self):
        n=now_utc()
        cand=fx("candidate",900,source="telegram",when=n)
        self.put(cand)
        parsed={"brand":cand["brand"],"model":cand["model"],"trim":cand["trim"],
        ...
        e=Execution(178,start,finish,"scheduled",1)
        with db(self.m) as c:
            rep=process_execution(c,self.s,self.o,e,force_candidates=False)
        self.assertEqual(rep["changed_candidates"],0)
        self.assertEqual(rep["evaluated"],0)
