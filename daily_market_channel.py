...
def parse_posts(page_html: str) -> list[SourcePost]:
    ...
    for message in soup.select(".tgme_widget_message[data-post]"):
        ...
        if stamp is None:
            continue
        # Telegram wraps inline formatting, links and custom emoji in nested tags.
        # get_text("\n") would incorrectly turn every inline node into a separate
        # visual line. Preserve only the source's real <br> line breaks.
        body_copy = BeautifulSoup(str(body), "html.parser").select_one(".tgme_widget_message_text")
        if body_copy is None:
            continue
        for br in body_copy.find_all("br"):
            br.replace_with("\n")
        text = html_lib.unescape(body_copy.get_text("", strip=False))
...
def _compact_visual_lines(text: str) -> str:
    """Keep each logical source row on one line and at most one blank separator."""
    output: list[str] = []
    blank = False
...
def format_market_rates(post: SourcePost) -> str:
    body = _compact_visual_lines(_replace_source_signature(post.text))
    return body.rstrip() + "\n\n" + support_footer()
...
def self_test() -> None:
    ...
    <div class="tgme_widget_message" data-post="DO_L4/100">
      <div class="tgme_widget_message_text"><b>نرخ فروش</b> <a>#دلار</a>، <a>#ارز</a>، <a>#سکه</a> و <a>#طلا</a> در بازار<br><br><span>💵 دلار:</span> <b>232,410 تومان</b> <span>1720🔼 %0.75+</span><br><span>💶 یورو:</span> <b>267,120</b> <span>2140🔼 %0.81+</span><br><br><span>💫 گرم ۱۸ عیار:</span> 24,520 هـ.تومان<br><span>💰</span> <a>@Dollar_Offical</a></div>
...
💰 @Dollar_Offical</div>
    ...
    rate_lines = transformed_rate.splitlines()
    if "💵 دلار: 232,410 تومان 1720🔼 %0.75+" not in rate_lines:
        raise AssertionError("Inline Telegram markup was incorrectly expanded into multiple lines")
    if "💶 یورو: 267,120 2140🔼 %0.81+" not in rate_lines:
        raise AssertionError("Compact currency row formatting failed")
    if "\n\n\n" in transformed_rate:
        raise AssertionError("Excessive blank lines remain in rate output")
