...
BLOCKED_FINANCE_PHRASES = (
    "اقساط", "اقساطی", "قسط", "قسطی", "لیزینگ", "پیش پرداخت",
    "پیشپرداخت", "مبلغ اولیه", "پرداخت اولیه", "ودیعه", "حواله",
    "پیش فروش", "ثبت نام", "ثبتنام", "وام", "چکی", "اعتباری",
    "معاوضه", "تهاتر", "شرایطی",
)
_NUM = r"[0-9]+(?:[,./][0-9]+)*"
...
def contains_finance_phrase(text: str) -> bool:
    ...
    return any(normalize_text(phrase) in normalized for phrase in BLOCKED_FINANCE_PHRASES)
...
def parse_vehicle_price(text: str, *, allow_shorthand: bool = True) -> tuple[Optional[int], str]:
    ...
    if contains_finance_phrase(text):
        return None, "blocked_finance_phrase"
...
def normalize_source_text(source: str, raw_text: str) -> SourceNormalization:
    ...
    return SourceNormalization(
        ...
        finance_blocked=contains_finance_phrase(text), price_reason=price_reason,
...
def run_self_test() -> None:
    ...
    prices = {
        ...
        "قیمت 1250\nقیمت 1250": 1_250_000_000,
        "قیمت ۱/۲۵۰\nپیش پرداخت ۳۰۰ میلیون": None,
        "قیمت 1250\nپرداخت اولیه 300 میلیون": None,
        "قیمت 1250\nاقساطی": None,
